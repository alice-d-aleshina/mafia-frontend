...
export default function NightShootComponent() {
  ...
  const handleShootClick = (num: number) => {
    ...
    const isSheriff = num === sheriffSelected;
    setMafiaStatus(isSheriff ? `Игрок ${num} шериф` : `Игрок ${num} не шериф`);
  }

  const handleConfirmShoot = () => {
    if (isSkipped) {
      router.push(`/mafia-game-sherif-check-night?roomId=${roomId}`);
  ...
  return (
    <div className="flex min-h-screen bg-[#1a1625] text-white">
      <div className="w-full max-w-md mx-auto p-4">
        ...
        <h3 className="text-center text-lg font-bold mb-4">Просыпается Дон и ищет Шерифа</h3>
